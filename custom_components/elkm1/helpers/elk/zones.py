...
from .message import az_encode, zb_encode, zd_encode, zp_encode, zs_encode, zt_encode, zv_encode
...
class Zones(Elements[Zone]):
    ...
    def sync(self) -> None:
        """Request alarm-by-zone, definitions, partitions, and statuses."""
    ...
    def _az_handler(self, alarm_status: list[ZoneAlarmState]) -> None:
        for zone in self.elements:
            zone.setattr("triggered_alarm", alarm_status[zone.index] != ZoneAlarmState.NO_ALARM, True)
    ...
    def _zb_handler(self, zone_number: int, zone_bypassed: bool) -> None:
        # A specific zone number gets a follow-up ZC (zone change) message.
        # Zone 000/999 (unbypass-all/bypass-all) doesn't identify which zones
        # changed, so re-request the full status instead.
        if zone_number < 0 or zone_number >= Max.ZONES.value:
            self._connection.send(zs_encode())

    def _zc_handler(self, zone_number: int, zone_status: tuple[ZoneLogicalStatus, ZonePhysicalStatus]) -> None:
    ...
    def _zs_handler(self, zone_statuses: list[tuple[ZoneLogicalStatus, ZonePhysicalStatus]]) -> None:
        for zone in self.elements:
