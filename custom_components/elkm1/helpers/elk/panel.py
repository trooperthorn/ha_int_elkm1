"""The overall panel: version, RTC, trouble status, ElkRP status, voice output."""
...
class Panel(Element):
    """The panel itself (index 0; there is only ever one)."""

    def __init__(self, connection: Connection, notifier: Notifier) -> None:
        super().__init__(0, connection, notifier)
        self.real_time_clock: str | None = None
        self.elkm1_version: str | None = None
        self.xep_version: str | None = None
        self.remote_programming_status: ElkRPStatus | int = 0
        self.system_trouble_status = ""
        self.temperature_units: str | None = None
        self.user_code_length: int | None = None
        self._configured = True
        self.setattr("name", "ElkM1", True)

        notifier.attach("VN", self._vn_handler)
        notifier.attach("XK", self._xk_handler)
        notifier.attach("RR", self._xk_handler)  # RR/XK carry the same RTC field.
        notifier.attach("RP", self._rp_handler)
        notifier.attach("SS", self._ss_handler)
        notifier.attach("UA", self._ua_handler)

    def sync(self) -> None:
        """Request version, temperature, and trouble status."""
        self._connection.send(vn_encode())
        self._connection.send(lw_encode())
        self._connection.send(ss_encode())
        # UA is not requested here - it's used as this integration's "sync complete" marker.

    def speak_word(self, word: int) -> None:
        """Speak a single word through the panel's voice/siren output."""
        self._connection.send(sw_encode(word))

    def speak_phrase(self, phrase: int) -> None:
        """Speak a phrase through the panel's voice/siren output."""
        self._connection.send(sp_encode(phrase))

    def set_time(self, datetime: dt.datetime | None = None) -> None:
        """Write the panel's real-time clock (defaults to the host's local time)."""
        if datetime is None:
    ...
    def _ua_handler(self, **kwargs: Any) -> None:
